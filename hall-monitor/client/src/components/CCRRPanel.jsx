...
function DomainCard({ domain, record, onSave, onEvidence, onRoadmap }) {
  ...
  return <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 overflow-hidden">
    ...
    {open && <div className="border-t border-slate-200 dark:border-slate-700 p-5 space-y-5">
      ...
      {path.length > 0 && <section className="rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-blue-950"><h5 className="font-semibold">Sequential advancement path</h5><ol className="mt-2 list-decimal pl-5 space-y-2">{path.map(item => <li key={item.from}><b>{item.from}→{item.to}:</b> {item.action}<button onClick={() => onRoadmap({ domain_id: domain.id, current_level: item.from, target_level: item.to, advancement_transition: `${item.from}→${item.to}`, advancement_action: item.action, gap_statement: `Advance ${domain.title} from Level ${item.from} to Level ${item.to}` })} className="ml-2 rounded border border-blue-300 px-2 py-0.5 text-xs font-medium hover:bg-blue-100">Add to roadmap</button></li>)}</ol></section>}
...
export default function CCRRPanel() {
  const [assessments, setAssessments] = useState([]); const [assessment, setAssessment] = useState(null); const [name, setName] = useState(''); const [priorAssessmentId, setPriorAssessmentId] = useState(''); const [expanded, setExpanded] = useState({ GOVERN: true }); const [status, setStatus] = useState('Loading');
  ...
  const create = async () => { if (!name.trim()) return; const data = await api.createCcrrAssessment({ name: name.trim(), prior_assessment_id: priorAssessmentId || null }); setAssessments(items => [data, ...items]); setAssessment(data); setName(''); setPriorAssessmentId(''); setStatus('CCRR assessment created'); };
  ...
  const addRoadmap = data => api.addCcrrRoadmapItem(assessment.id, data).then(refresh).catch(() => setStatus('Could not add roadmap action'));
